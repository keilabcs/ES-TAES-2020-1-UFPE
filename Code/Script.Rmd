...
library(ggplot2)
library(patchwork)
p1 <- ggplot(Survey, aes(x = Survey$build)) +
  geom_bar()

p2 <- ggplot(Survey, aes(x = Survey$mismanaging)) +
  geom_bar()

p3 <- ggplot(Survey, aes(x = Survey$rework)) +
  geom_bar()
...
Para cada 2(BACKLOG, BUILD) Arrays fazer um teste de Mann-Whitney. Se p<0.05 (Resultado significativo) considerar, se p>0.05 (Resultado não significativo) descartar.
Ex. BACKLOG < BUILD or REWORK < BUILD
Elaborate Rank. por resultado de Mann-Whitney Ex. BUILD - 2, REWORK - 1, BACKLOG - 1.
Exemplo de Rank: 1. BUIL
                 2. BACKLOG, REWORK

https://www.youtube.com/watch?v=ms3fMFBgH8A&feature=youtu.be

Mann-Whitney teste não parametrico usado em que a amostras não é normmal. 
Em amostras não normais(independentes) a média não tem boa representação.
Recortar a Mediana, o intervalo e a amplitude e distância interquartil. 
```{r}
#Teste de Mann-Whitney para duas amostras independentes: Bicaudal.
#Variavel dependente e variavel de agrupamento 
wilcox.test(build ~ mismanaging, data = Survey)
```
